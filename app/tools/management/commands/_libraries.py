    ...
    for index, row in df.iterrows():
        lib.append(
            Library(
                ...
                DDK_adults=True if row['Dorosli'] == 'tak' else False,
...
def importCoordinators():
    df = pd.read_excel('Biblioteki v3.xls',sheet_name='Koordynatorzy',index_col=0, header=0, na_values= str)
    lib = []
    for index, row in df.iterrows():
        if row['Biblioteka'] == 0: continue
        lib.append(
            Library(
                area=row['Miejscowość'],
                city='Koordynator',
                name=row['Biblioteka'],
                address1=row['Adr 1'],
                address2='',
                phone=row['tel'],
                moderator=row['moderatorka'],
                email=row['email'],
                www='',
                salutation='',
                DDK_kids=False,
